...
def view_setting(topMsg):
    ...
    msg1 = tk.Label(top, text="API 秘钥：",font=("100px"))
    ...
    msg2 = tk.Label(top, text="站点（加 https:// ）：",font=(10))
...
def create_SendArticle(event):
    ...
    msg = tk.Label(top2, text="下一版敬请期待！",bg='#9BCD9B',font=(15))
...
def note_view():
    ...
    width = 500
    height = 400
    screenwidth = window.winfo_screenwidth()*(1/0.75)
    screenheight = window.winfo_screenheight()*(1/0.75)
    ...
    # 文本输入框属性
    text = tk.Text(window, width=50, font = tf.Font(size=14), fg ="#5a5c69", height=9,
    ...
    # 按钮 
    button = tk.Button(window, text="笔记发表", font = tf.Font(size=14), command=note_callback)
    button.pack(pady = "10px", ipadx = "7px", ipady = "1px")
